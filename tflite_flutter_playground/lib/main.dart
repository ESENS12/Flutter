...
class _MyAppState extends State<MyApp> {
  ...
  onSelect(model) async {
    setState(() {
      _busy = true;
      _model = model;
      // todo cannot clear a fixed-length list
      // _recognitions.clear();
    });
    await loadModel();
  ...
  Widget build(BuildContext context) {
    ...
    if (_model == deeplab && bIsRecognition) {
      ...
      stackChildren.add(Positioned(
        ...
        left: 0.0,
        width: size.width,
        child: !hasImage ? Text('No image selected.') : Image.file(_image),
      ));
    }
